...
# [1] "TCGA-PRAD:12371,2588"
# [1] "TCGA-COAD:16823,5090"
# [1] "TCGA-LUAD:11165,418"
# [1] "TCGA-LUSC:16225,3470"
# [1] "TCGA-BRCA:18025,3088"
...
sum(alluniqprobes %in% rownames(alltumor)) #14320
sum(comprobes %in% rownames(alltumor)) #34345
...
sum(comprobes %in% rownames(allTCGAdat)) #33103
...
save(cvfit,file="result/cvfit6cancers_all.RData") #tumors and normal
...
table(y,resps)
# resps
# y           TCGA-BRCA TCGA-COAD TCGA-LIHC TCGA-LUAD TCGA-LUSC TCGA-NORM TCGA-PRAD
# TCGA-NORM         7         0         0         0         0       291        10
# TCGA-PRAD         0         0         0         0         0         9       489
# TCGA-COAD         0       296         0         0         0         0         0
# TCGA-LUAD         0         0         0       449         5         4         0
# TCGA-LUSC         0         0         0        12       356         2         0
# TCGA-BRCA       778         0         0         0         0         4         0
# TCGA-LIHC         0         0       369         0         0         8         0
...
# TCGA-BRCA TCGA-COAD TCGA-LIHC TCGA-LUAD TCGA-LUSC TCGA-NORM TCGA-PRAD 
# 0         0         0         0         0        38       487 
...
# #CIT-PRAD sample (normals are not available, skip)
# CIT_PRAD=as.data.frame(fread("data/PRAD/E-MTAB-6131/Methylation_bval.txt"))
# if ("Target_ID" %in% colnames(CIT_PRAD)) 
# {
#   rownames(CIT_PRAD)=CIT_PRAD$Target_ID
#   CIT_PRAD=CIT_PRAD[,-1]
# }
# CIT_sampletable=read.table("data/PRAD/E-MTAB-6131/E-MTAB-6131.sdrf.txt",sep="\t")
# colnames(CIT_sampletable)=CIT_sampletable[1,]
# CIT_sampletable=CIT_sampletable[-1,]
# idx=match(colnames(CIT_PRAD),CIT_sampletable$`Source Name`)
# table(CIT_sampletable$`Characteristics[sampling site]`[idx])

#GEO-BRCA-NORM
GEO_BRCA_NORM=as.data.frame(fread("data/BRCA/GSE101961/GSE101961_series_matrix.txt",skip=67)) #black and white
rownames(GEO_BRCA_NORM)=GEO_BRCA_NORM$ID_REF
GEO_BRCA_NORM=GEO_BRCA_NORM[,-1]
GEO_BRCA_NORM_x=form_xpredict(dat2=t(GEO_BRCA_NORM)) #0 probes not available
GEO_BRCA_NORM_predictprob=predict(cvfit, s=lambda.best,newx = GEO_BRCA_NORM_x, type = "response")
GEO_BRCA_NORM_predictcancer=predit_cancertype(allprobs=GEO_BRCA_NORM_predictprob)
table(GEO_BRCA_NORM_predictcancer$class)
# TCGA-NORM 
# 121

#GEO-LUNG-NORM1
GEO_LUNG_NORM1=as.data.frame(fread("data/LUNG/GSE52401/GSE52401_series_matrix.txt",skip=65))
rownames(GEO_LUNG_NORM1)=GEO_LUNG_NORM1$ID_REF
GEO_LUNG_NORM1=GEO_LUNG_NORM1[,-1]
GEO_LUNG_NORM1_x=form_xpredict(dat2=t(GEO_LUNG_NORM1)) #0 probes not available
GEO_LUNG_NORM1_predictprob=predict(cvfit, s=lambda.best,newx = GEO_LUNG_NORM1_x, type = "response")
GEO_LUNG_NORM1_predictcancer=predit_cancertype(allprobs=GEO_LUNG_NORM1_predictprob)
table(GEO_LUNG_NORM1_predictcancer$class)
# TCGA-LUAD TCGA-LUSC TCGA-NORM 
# 13        9       222 
...
Grady_COLON_NORM=MEall[,names(MEall) %in% c(LowID,MedID,HighID)]
Grady_COLON_NORM_x=form_xpredict(dat2=t(Grady_COLON_NORM)) #20 probes not available
Grady_COLON_NORM_predictprob=predict(cvfit, s=lambda.best,newx = Grady_COLON_NORM_x, type = "response")
Grady_COLON_NORM_predictcancer=predit_cancertype(allprobs=Grady_COLON_NORM_predictprob)
table(Grady_COLON_NORM_predictcancer$class)
# TCGA-LUSC TCGA-NORM 
# 1       127 
#for the sample with mistake class
which(Grady_COLON_NORM_predictcancer$class=="TCGA-LUSC")
which(rownames(clinicaltable)=="GSM3892447")
clinicaltable$risk[285] #Low 
alldat=form_xpredict1(dat2=t(Grady_COLON_NORM))
Grady_COLON_NORM_predictcancer_knn=knnclust()
table(Grady_COLON_NORM_predictcancer_knn$class)
# TCGA-BRCA TCGA-COAD TCGA-LUAD TCGA-LUSC TCGA-NORM TCGA-PRAD 
# 0         0         0         0       128         0 

#GEO_COAD
GEO_COAD=as.data.frame(fread("data/COAD/GSE77718/GSE77718_series_matrix.txt",skip=65))
rownames(GEO_COAD)=GEO_COAD$ID_REF
GEO_COAD=GEO_COAD[,-1]
sampletable=read.table("data/COAD/GSE77718/sampletable.txt")
...
GEO_COAD_predictcancer=predit_cancertype(allprobs=GEO_COAD_predictprob)
idx=match(colnames(GEO_COAD),sampletable$V1)
table(sampletable$type[idx],GEO_COAD_predictcancer$class)
#         TCGA-COAD TCGA-NORM
# Normal         3        93
# Tumor         93         3
alldat=form_xpredict1(dat2=t(GEO_COAD))
GEO_COAD_predictcancer_knn=knnclust()
table(sampletable$type[idx],GEO_COAD_predictcancer_knn$class)
#           TCGA-BRCA TCGA-COAD TCGA-LIHC TCGA-LUAD TCGA-LUSC TCGA-NORM TCGA-PRAD
# Normal         0         4         0         0         0        92         0
# Tumor          0        93         0         0         0         3         0

#GEO-PRAD
GEO_PRAD=as.data.frame(fread("data/PRAD/GSE76269/GSE76938_matrix_processed.txt"))
...
sampletable=read.table("data/PRAD/GSE76269/sampletable.txt")
sampletable$type="Tumor"
...
GEO_PRAD_predictcancer=predit_cancertype(allprobs=GEO_PRAD_predictprob)
idx=match(colnames(GEO_PRAD),sampletable$V2)
table(sampletable$type[idx],GEO_PRAD_predictcancer$class)
#            TCGA-NORM TCGA-PRAD
# Normal        54         9
# Tumor          4        69

alldat=form_xpredict1(dat2=t(GEO_PRAD))
GEO_PRAD_predictcancer_knn=knnclust()
table(sampletable$type[idx],GEO_PRAD_predictcancer_knn$class)
# TCGA-BRCA TCGA-COAD TCGA-LIHC TCGA-LUAD TCGA-LUSC TCGA-NORM TCGA-PRAD
# Normal         0         0         0         0         0        51        12
# Tumor          0         0         0         0         0         3        70


#GEO_Liver
GEO_LIVE=as.data.frame(fread("data/LIHC/GSE54503/GSE54503_series_matrix.txt",skip=59))
rownames(GEO_LIVE)=GEO_LIVE$ID_REF
GEO_LIVE=GEO_LIVE[,-1]
sampletable=read.table("data/LIHC/GSE54503/sampletable.txt",sep="\t")
...
GEO_LIVE_predictcancer=predit_cancertype(allprobs=GEO_LIVE_predictprob)
idx=match(colnames(GEO_LIVE),sampletable$V1)
table(sampletable$V3[idx],GEO_LIVE_predictcancer$class)
#                            TCGA-LIHC TCGA-NORM
# adjacent non-tumor tissue         3        63
# HCC tumor                        65         1