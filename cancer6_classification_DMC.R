#!/usr/bin/env Rscript
#6 cancer classification include normals, DMC sig probes 
...
for (i in 1:length(Cancertypes))
{
  ...
  tmp=rownames(output)[output[,18]<0.05/nrow(output) & bdiff>0.05 & output[,4]<0.1 & output[,7]>0.05]
...
length(comprobes) #26511
for (i in 1:length(sigprobes))
{
  print(paste0(Cancertypes[i],":",length(sigprobes[[i]]),",",length(uniqprobes[[i]])))
}
#total significant probes,unique probes:
# [1] "TCGA-PRAD:8091,2320"
# [1] "TCGA-COAD:7873,3098"
# [1] "TCGA-LUAD:5029,245"
# [1] "TCGA-LUSC:9010,2872"
# [1] "TCGA-BRCA:14823,5236"
# [1] "TCGA-LIHC:2822,598"
alluniqprobes=NULL
for (i in 1:length(sigprobes))
{
  tmp=uniqprobes[[i]]
...
sum(alluniqprobes %in% rownames(alltumor)) #12446
sum(comprobes %in% rownames(alltumor)) #23468
...
sum(comprobes %in% rownames(allTCGAdat)) #22700
...
save(cvfit,file="result/cvfit6cancersDMC_all.RData") #tumors and normal
...
# Sys.time()
#dev.off()
lambda.best=cvfit$lambda.min #215
...
# TCGA-PRAD         0         0         0         0         0         8       490
# TCGA-COAD         0       296         0         0         0         0         0
# TCGA-LUAD         0         0         0       449         5         4         0
# TCGA-LUSC         0         0         0        14       354         2         0
# TCGA-BRCA       778         0         0         0         0         4         0
# TCGA-LIHC         0         0       368         0         0         9         0
...
# TCGA-COAD TCGA-LUAD TCGA-LUSC TCGA-NORM 
# 54         1         1         8 
...
Grady_COLON_NORM_x=form_xpredict(dat2=t(Grady_COLON_NORM)) #26 probes not available
